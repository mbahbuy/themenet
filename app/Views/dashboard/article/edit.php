              ...
              <div class="card-body">
                <div class="row mb-3">
                  <div class="col-12">
                    ...
                    <input id="inpjudul" type="text" class="form-control" maxlength="100" name="judul" value="<?= $article['judul'] ?>" />
                    <input type="hidden" id="id-judul" value="<?= $article['id_judul'] ?>">
                ...
                <div class="row mb-3" >
                  <div class="col-md-12 mb-3 text-center">
                    <img src="<?= filter_var($article['gambar'], FILTER_VALIDATE_URL) == true ? $article['gambar'] : url_to('/') . 'images/gallery/' . $article['gambar'] ?>" alt="poster" class="img-fluid">
                    <input type="hidden" id="old-poster" value="<?=  $article['gambar'] ?>">
                  </div>
                  <div class="col-md-12">
                ...
                <div class="row mb-3">
                  <div class="col-12">
                    <textarea name="konten" id="body_konten"><?= $article['konten'] ?></textarea>
                  </div>
                </div>
                <!-- end row -->
              </div>
              ...
              <div class="card-body">
                <div class="row mb-3">
                  <div class="col-12">
                    ...
                    <select class="form-control" style="width: 100%;height:100%;" id="kategori">
                      <?php if(sizeof($category)) : ?>
                          <?php foreach($category as $item) : ?>
                            <option value="<?= $item['id_kategori'] ?>" <?= $item['nama_kategori'] == $article['kategori'] ? 'selected' : '' ?> ><?= $item['nama_kategori'] ?></option>
                          <?php endforeach ?>
                      <?php endif ?>
                    </select>
                  </div>
                ...
                <div class="row mb-3">
                  <div class="col-12">
                    ...
                    <textarea id="inpdeskripsi" class="form-control" maxlength="225" rows="3"><?= $article['konten_singkat'] ?></textarea>
                  </div>
                  <!-- end col -->
                </div>
                <!-- end row -->
                <div class="row mb-3">
                  <div class="col-12">
                    ...
                    <textarea id="meta_keyword" class="form-control" maxlength="225" rows="3"><?= $article['meta_keyword'] ?></textarea>
                  </div>
                  <!-- end col -->
                </div>
                <!-- end row -->
                    ...
                    <select id="select-meta" multiple="multiple" data-placeholder="Pilih tag" style="width: 100%">
                      <?php if(sizeof($tags)): ?>
                        <?php foreach($tags as $tag): ?>
                          <?php $selected = '';
                            foreach($article['meta'] as $tagA){
                              if ($tagA['slug'] == $tag['slug']) {
                                $selected =  'selected';
                              }
                            }?>
                          <option value="<?= $tag['id_meta'] ?>" <?= $selected ?>><?= $tag['meta'] ?></option>
                        <?php endforeach; ?>
                      <?php endif; ?>
                    </select>
                  </div>
                ...
                <div class="card-body">
                  <div class="row">
                    <div class="col-12 text-center mb-3">
                        ...
                        <?php if($article['status'] == 'D') : ?>
                          <button id="btnpublish" onclick="publishclick()" type="button" class="btn btn-success waves-effect waves-light" title="Publish sekarang">
                    ...
                    <?php if($article['status'] == 'D') : ?>
                          <div class="col-12 text-center mt-3" id="time-taker">
            ...
            <div class="card mt-5">
                <div class="card-body">
                    <div class="row">
                      <?php if($article['status'] == 'R'): ?>
                        <div class="col-12 text-center mb-3">
                            <label class="mb-1 fw-medium text-muted">Restore Artikel?</label>
                        </div>
                        <div class="col-12 text-center mt-3" >
                            <form action="<?= url_to('article.restore', $article['slug']) ?>" method="post" class="d-inline">
                                <?= csrf_field(); ?>
                                <input type="hidden" name="_method" value="PUT">
                                <button type="submit" class="btn btn-info btn-sm d-inline" onclick="return confirm('anda berkenan mengambil artikel ini dari tempat sampah?');"><i class="fas fa-sync-alt"></i> Restore</button>
                            </form>
                        </div>
                      <?php else: ?>
                        <div class="col-12 text-center mb-3">
                            <label class="mb-1 fw-medium text-muted">Hapus Artikel?</label>
                        </div>
                        <div class="col-12 text-center mt-3" >
                            <form action="<?= url_to('article.delete', $article['slug']) ?>" method="post" class="d-inline">
                                <?= csrf_field(); ?>
                                <input type="hidden" name="_method" value="DELETE">
                                <button type="submit" class="btn btn-danger btn-sm d-inline" onclick="return confirm('anda berkenan menghapus artikel ini?');"><i class="fas fa-trash"></i> Hapus</button>
                            </form>
                        </div>
                      <?php endif; ?>
                    </div>
...
<?= $this->section('script') ?>
  <!-- Select2 -->
  <script src="<?= url_to('/') ?>/template/adminLTE/plugins/select2/js/select2.full.min.js"></script>
  <script>
    $(function () {
        tinymce.remove('#body_konten');
              ...
              if (response.length !== 0) {             
                for (let i = 0; i < response.length; i++) {
                  $('#backlink-table').append(`
                        <tr>
                            <td>${i + 1}</td>
                            <td class="title-article">${response[i].judul}</td>
                            <td>
                                <button type="button" class="btn btn-default btn-sm button-delete-related-article" data-judul="${response[i].judul}" data-id_judul="${response[i].id_judul}" title="hapus dari related article">
        ...
        $.ajax({
            ...
            success: function (response) {
              if (response.lenght !== 0) {
                for (let i = 0; i < response.length; i++) {
                  if ($('#backlink-table').find(`button.button-delete-related-article[data-id_judul="${response[i].id_judul}"]`).length == 1 || $('#id-judul').val() == response[i].id_judul) {} else {
    ...
    {
        const imagePre = $(img).closest(".row").find(".img-fluid");
    ...
    function simpanclick(){
      ...
      else if(poster == null && oldPoster == null){
      ...
      if(uploadStatus !== 1 ){}
        ...
        $.ajax({
            type: 'POST',
            url: "<?= url_to('article.update', $article['slug']) ?>",
            data: formData,
            contentType: false,
            processData: false,
            success: function(response) {
              makeToast(response.bg, response.message);
              if (response.bg == 'bg-success') {if(confirm('Pindah ke list artikel?')){location.href= "<?= url_to('article.index') ?>"}}
              if ($('#time-taker').length == 1 && $('#publish-date').val() !== null || $('#time-taker').length == 1 && $('#publish-date').val() !== '') {
                $('#btnpublish').remove();
                $('#time-taker').remove();
              }
            },
    ...
    function publishclick()
    {
      ...
      if(uploadStatus !== 1 ){}
        ...
        $.ajax({
            type: 'POST',
            url: "<?= url_to('article.publish_update', $article['slug']) ?>",
            data: formData,
            contentType: false,
            processData: false,
            success: function(response) {
              makeToast(response.bg, response.message);
              if (response.bg == 'bg-success') {if(confirm('Pindah ke list artikel?')){location.href= "<?= url_to('article.index') ?>"} else {
                $('#btnpublish').remove();
                $('#time-taker').remove();
              }}
            },
